...
export class TableRow extends Component {
  ...
  render() {
    const { headers, row, settings, callbacks, cell_components, ...rest } = this.props;
        ...
        {cell_components.map((ChildComponent, index) => {
          ...
          return (
            <TableCell row={row} key={cell_key} header={headers[index]}>
              <ChildComponent row={row} column_index={index} header={headers[index]} callbacks={callbacks} {...rest} />
            </TableCell>
          );
        })}
      </div>
